...
const T = {
    ...
    table: "payments",
    alias: "p",
    select: ["id", "order_id", "payment_method", "payment_date", "amount", "status", "payment_ref", "created_at", "updated_at"],
    allowedFilters: ["order_id", "status", "payment_method"],
    allowedSort: ["created_at", "amount"],
    mutable: ["order_id", "payment_method", "payment_date", "amount", "status", "payment_ref"],
};

function q(c) { return T.alias ? `${T.alias}.${c}` : c; }
...
const paymentsService = {
    ...
    async remove(id) {
        const sql = `UPDATE ${T.table} SET deleted_at=now(),updated_at=now(),status='INACTIVE' WHERE id=$1 AND deleted_at IS NULL`;
