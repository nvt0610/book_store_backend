...
const orderItemsController = {
  async list(req, res) {
    try { return R.ok(res, await orderItemsService.list(req.query)); }
    catch (err) { return R.internalError(res, err.message); }
  },
  async getById(req, res) {
    const { id } = req.params;
    if (!isUuid(id)) return R.badRequest(res, "Invalid UUID");
    const row = await orderItemsService.getById(id, req.query.showDeleted);
  ...
  async create(req, res) {
    try {
      ...
      const created = await orderItemsService.create(req.body);
  ...
  async update(req, res) {
    const { id } = req.params;
    if (!isUuid(id)) return R.badRequest(res, "Invalid UUID");
    const updated = await orderItemsService.update(id, req.body);
    return updated ? R.ok(res, updated) : R.notFound(res, "OrderItem not found");
  },
  async remove(req, res) {
    const { id } = req.params;
    if (!isUuid(id)) return R.badRequest(res, "Invalid UUID");
    const ok = await orderItemsService.remove(id);
